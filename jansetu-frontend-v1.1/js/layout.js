...
(function (w, d) {
  ...
  function initNav() {
    ...
    /* Crossing the 900px breakpoint changes how the nav is drawn (panel vs. inline bar);
       closing it on every crossing stops a menu opened on mobile from reappearing "open"
       if the viewport is later resized back down. */
    var mq = w.matchMedia('(min-width: 56.25rem)');
    var onCross = function () { setMenu(false); };
    if (mq.addEventListener) mq.addEventListener('change', onCross); else if (mq.addListener) mq.addListener(onCross);
  }
