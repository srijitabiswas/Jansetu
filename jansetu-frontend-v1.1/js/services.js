...
(function (w, d) {
  ...
  function openModal(id, trigger) {
    ...
    d.body.style.overflow = 'hidden';                      // stop the page behind the dialog from scrolling
  }
  function closeModal() { if (modal.close) modal.close(); else modal.removeAttribute('open'); d.body.style.overflow = ''; }
  ...
  /* Manual focus-trap safety net. showModal() already traps focus in modern browsers;
     this guarantees the same behaviour everywhere by cycling Tab between the dialog's
     first and last focusable elements. */
  modal.addEventListener('keydown', function (e) {
    if (e.key !== 'Tab') return;
    var items = JS.$$('a[href], button:not([disabled])', modal);
