...
(function (w, d) {
  ...
  function validateField(f, value) {
    ...
    switch (f.kind) {
      ...
      case 'whole':    return V.wholeNumber(v) ? '' : 'Enter a whole number, for example 120000.';
      case 'pastDate': return V.dateNotFuture(v) ? '' : 'The date cannot be in the future.';
      case 'dob':
  ...
  $('#copy-ref').addEventListener('click', function () {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(text).then(function () { JS.toast('Reference ID copied'); });
    else { var r = d.createRange(); r.selectNodeContents($('#ref-code')); var sel = w.getSelection(); sel.removeAllRanges(); sel.addRange(r); JS.toast('Press Ctrl+C to copy'); }
  });
  ...
  var draft = JS.store.get('draft', null), wanted = JS.params().get('service');
  if (draft && draft.serviceId && (!wanted || wanted === draft.serviceId)) {
    state.serviceId = draft.serviceId; state.values = draft.values || {}; state.elig = draft.elig || []; state.files = draft.files || {};
    $('#restore-note').hidden = false;
  } else if (wanted && JS.getService(wanted)) { state.serviceId = wanted; }
  $('#service').value = state.serviceId;
  renderEligibility();
  goTo(draft && !wanted && draft.step ? draft.step : 1);
})(window, document);
